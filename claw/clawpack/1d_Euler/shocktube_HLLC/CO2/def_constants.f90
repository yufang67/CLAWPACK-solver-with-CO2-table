...
! Critical Point 
  ...
  REAL(pr), PARAMETER :: c_cr   = 140_pr              ! singularity in EoS, average of the previous (T,v)
! Triple Point 
  REAL(pr), PARAMETER :: T_tri  = 216.592_pr          ! triple point temperature [K]
  REAL(pr), PARAMETER :: P_tri  = 517964.343_pr       ! triple point pressure    [Pa]
! e = h-pv
  ...
  REAL(pr), PARAMETER :: rho_tri_L  = 1178.4_pr       ! liquid triple point density              [kg/m3]
...
!  The U maximum point along the saturation curve
  ...
  REAL(pr), PARAMETER :: v_umax   = 2.003045597e-2_pr ! Umax specific volume [m3/kg]
...
!  REAL(pr), PARAMETER :: T_B      = 316.44_pr
!  REAL(pr), PARAMETER :: v_B      = 1.8266e-3_pr
...
! Left boundary isobar
   REAL(pr), PARAMETER :: p_max = 10.0e6_pr
! Residu error
   REAL(pr), PARAMETER :: res_ref = 1.0e-4_pr
! Maximum internal energy corrsponds to 500K [J/kg]
   REAL(pr), PARAMETER :: u_end = 68569_pr 
! à definir  ??
!   REAL(pr), PARAMETER :: u_end_TP = 10.0e6_pr
!   REAL(pr), PARAMETER :: T_hp_TPH = 10.0e6_pr
...
!   INTEGER, PARAMETER :: NNN_TPL = 20, MMM_TPL = 20, NNN_sat_TPL = 3*(NNN_TPL-1)+1
!   INTEGER, PARAMETER :: NNN_TPM = 20, MMM_TPM = 20, NNN_sat_TPM = 3*(NNN_TPM-1)+1
!   INTEGER, PARAMETER :: NNN_TPH = 20, MMM_TPH = 20, NNN_sat_TPH = 3*(NNN_TPH-1)+1
!
! NUMBER OF POINTS ON THE SATURATION CURVE FOR THE TWO-PHASE ANALYSIS
   INTEGER, PARAMETER :: NNN_TP = 100, NNN_sat_TP = 3*(NNN_TP-1)+1
!
