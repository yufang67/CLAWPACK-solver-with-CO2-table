...
!        print*, i, abs(y_mesh_LL(i) - energy ) / y_mesh_LL(i)
              IF (res_LL(i,1) > res_ref) THEN
                print*, "left_res", res_LL(i,1), "iter", Niter,"flag",exitflag
                STOP
              ENDIF
...
!         print*, vvv_LL(NNN_LL-1,MMM_LL), y_mesh_LL(NNN_LL-1), & 
!        & ppp_LL(NNN_LL-1,MMM_LL), TTT_LL(NNN_LL-1,MMM_LL)
!         stop
...
!
        ...
        DO i=2, NNN_sat_LL
              ...
              v_guess_sat = v_Lsat_LL(i-1)
              vap_guess_sat = vap_sat_LL(i-1)
!         
...
!      T_guess_sat = T_tri
!      v_guess_sat = v_tri_L
!DO i = 1, NNN_sat_LL-1 !Metastable conditions
!
...
!
              IF (res_pmax_LL(i) > res_ref) THEN
                print*, "left_pmax_res", res_pmax_LL(i), "iter", Niter
                STOP
              ENDIF
...
DO i = 1, NNN_LL-1
   j = 3 * (i-1) + 1
   CALL polyfit(spline_Lsat_LL(:,i), y_mesh_sat_LL(j:j+3),v_Lsat_LL(j:j+3), ord_spline)
!   CALL polyfit(spline_Lsat_LL(:,i), y_mesh_sat_LL(j:j+3),LOG10(v_Lsat_LL(j:j+3)), ord_spline)
!   CALL polyfit(spline_meta_LL(:,i), y_mesh_sat_LL(j:j+3),v_liq_meta(j:j+3), ord_spline)
   CALL polyfit(spline_pmax_LL(:,i), y_mesh_sat_LL(j:j+3),v_Lpmax_LL(j:j+3), ord_spline)
ENDDO
!
!
!
