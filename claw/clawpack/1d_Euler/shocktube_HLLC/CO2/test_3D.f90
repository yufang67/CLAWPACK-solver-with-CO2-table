...
!
!======================================================================
!      REAL(pr) :: p, T, c, x, a, res, u_in, v_in,p_ref,T_ref,v_l,v_v,&
!&                 resnorm,in_2, vl_guess,vv_guess,e_v,e_l,p_guess,x_out,&
!&                 delta
!      INTEGER :: Niter, exitflag,i,j,k
!      REAL(pr), DIMENSION(100) :: T_tab,p_tab 
!      REAL(pr), DIMENSION(101) :: x_tab
!      REAL(pr) :: Perr,Terr,xerr       
!
!      delta = (T_cr-0.1_pr - (T_tri+0.01_pr))/ (100-1)
!      T_tab = T_tri+0.01_pr + (/(i*delta,i=0,100-1)/)
!      delta = 1.0_pr / (101-1)
!      x_tab = 0.01_pr + (/(i*delta,i=0,101-1)/)
...
!WRITE(22,*), T_tab(i), x_tab(j), Perr


!   print*, "error for T=", abs(T-T_tab(i))/T_tab(i) 
!   print*, "error for p=", abs(p-p_tab(i))/p_tab(i)
