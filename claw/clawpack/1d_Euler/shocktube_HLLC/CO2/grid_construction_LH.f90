...
!--------------------------------------------------
!
! Initial guess for point (1,1) and (1,MMM_LH)
!
...
!
              IF (res_LH(i,1) > res_ref) THEN
                print*, "left_res", res_LH(i,1), "iter", Niter,"flag",exitflag
                STOP
              ENDIF
...
!
           ...
           IF (res_LH(i,MMM_LL) > res_ref) THEN
                print*, "right_res", res_LH(i,MMM_LH), "iter", Niter
                STOP
           ENDIF
...
!         
              ...
              & vliq_guess,v_guess(i-1,MMM_LH),T_guess(i-1,MMM_LH))
!
              CALL pressure(TTT_LH(i,MMM_LH),vvv_LH(i,MMM_LH),press)
              ppp_LH(i,MMM_LH) = press
!
...
!                
!              CALL New_Rap3D(4,vliq_LH(i),vvv_LH(i,MMM_LH),TTT_LH(i,MMM_LH),&
!              & res_LH(i,MMM_LH), Niter, exitflag, y_mesh_LH(i),&
!              & vliq_guess,v_guess2,T_guess2)
              
!              print*, 'found i', i, vliq_LH(i),vvv_LH(i,MMM_LH),TTT_LH(i,MMM_LH)
...
!
!OPEN (UNIT = 22,             FILE = 'ev_sat_domain_LH.mgr', &
!&   FORM = 'formatted',     ACTION = 'write',   &
!&   STATUS = 'replace')
!
!DO i = 1, NNN_LH 
!     WRITE(22,*), vvv_LH(i,MMM_LH), y_mesh_LH(i) 
!ENDDO
...
!
        ...
        v_guess_sat = v_Lsat_LH(1) + 1e-4_pr
        vliq_guess_sat = vliq_sat_LH(1) - 1e-4_pr
!
        DO i=2, NNN_sat_LH-1
...
!
              IF (res_sat_LH(i) > res_ref) THEN
                print*, "right_sat_res_LH", res_sat_LH(i), "iter", Niter, 'row', i
!                STOP
              ENDIF
!
        END DO
...
!
              IF (res_pmax_LH(i) > res_ref) THEN
                print*, "left_pmax_res", res_pmax_LH(i), "iter", Niter
                STOP
              ENDIF
              T_guess(i+1,1) = T_sat_pmax(i)
              v_guess(i+1,1) = v_Lpmax_LH(i)
!                
         END DO
