...
!LL
REAL(8), DIMENSION(NNN_test_LL, MMM_test_LL) :: v_test_LL_log,v_test_LL, c_BL_LL, &
&  p_BL_LL,  T_BL_LL, x_out_LL, p_LOIET_LL, T_LOIET_LL, c_LOIET_LL, &
&  err_pBL_LL, err_TBL_LL, err_cBL_LL!, c_BC_LL, p_BC_LL,  T_BC_LL,  &
!&  err_pBC_LL, err_TBC_LL, err_cBC_LL
!
!LH
REAL(8), DIMENSION(NNN_test_LH, MMM_test_LH) :: v_test_LH_log,v_test_LH, c_BL_LH, &
&  p_BL_LH,  T_BL_LH, x_out_LH, p_LOIET_LH, T_LOIET_LH, c_LOIET_LH, &
&  err_pBL_LH, err_TBL_LH, err_cBL_LH!, c_BC_LH, p_BC_LH,  T_BC_LH,  &
!&  err_pBC_LH, err_TBC_LH, err_cBC_LH, v_test_LH_log
!
!R
REAL(8), DIMENSION(NNN_test_R, MMM_test_R) :: v_test_R_log, v_test_R, c_BL_R, &
&  p_BL_R,  T_BL_R, x_out_R, p_LOIET_R, T_LOIET_R, c_LOIET_R, &
&  err_pBL_R, err_TBL_R, err_cBL_R!, c_BC_R, p_BC_R,  T_BC_R,  &
!&  err_pBC_R, err_TBC_R, err_cBC_R, v_test_R_log
!HT
REAL(8), DIMENSION(NNN_test_HT, MMM_test_HT) :: v_test_HT_log, v_test_HT, c_BL_HT, &
&  p_BL_HT,  T_BL_HT, x_out_HT, p_LOIET_HT, T_LOIET_HT, c_LOIET_HT, &
&  err_pBL_HT, err_TBL_HT, err_cBL_HT!, c_BC_HT, p_BC_HT,  T_BC_HT,  &
!&  err_pBC_HT, err_TBC_HT, err_cBC_HT, v_test_HT_log
   
REAL(8) :: v_min_li, v_max_li, out_2
!
!
!
!
...
!
!
      IF (err_pBL_LL(k,p) > 1_pr) THEN
         print*, "press_error" , err_pBL_LL(k,p),"row", k,"col", p,res
         print*, "p_interp",p_BL_LL(k,p),"p_eos",p_LOIET_LL(k,p),T_LOIET_LL(k,p), T_BL_LL(k,p),v_test_LL(k,p),y_test_LL(k)
!         STOP
      ENDIF
         WRITE(41,*) y_test_LL(k), v_test_LL(k,p), err_pBL_LL(k,p)
         WRITE(57,*) y_test_LL(k), v_test_LL(k,p), err_TBL_LL(k,p)
         WRITE(63,*) y_test_LL(k), v_test_LL(k,p), err_cBL_LL(k,p)
...
!        & flag, y_test_LH(k), T_LOIET_LH(k,p), v_test_LH(k,p), out3)
        ...
        CALL pressure(T_BL_LH(k,p),v_test_LH(k,p),p_LOIET_LH(k,p))
!
        CALL sound_speed(T_BL_LH(k,p),v_test_LH(k,p), c_LOIET_LH(k,p))
!
        IF (res > 1d-8) THEN
...
!
         ...
         WRITE(57,*) y_test_LH(k), v_test_LH(k,p), err_TBL_LH(k,p)
         WRITE(63,*) y_test_LH(k), v_test_LH(k,p), err_cBL_LH(k,p)
...
!
         ...
         WRITE(57,*) y_test_R(k), v_test_R(k,p), err_TBL_R(k,p)
         WRITE(58,*) y_test_R(k), v_test_R(k,p), err_cBL_R(k,p)
...
!
         ...
         WRITE(57,*) y_test_HT(k), v_test_HT(k,p), err_TBL_HT(k,p)
         WRITE(58,*) y_test_HT(k), v_test_HT(k,p), err_cBL_HT(k,p)
      !   WRITE(64,*) y_test_HT(k), v_test_HT(k,p), p_BL_HT(k,p) 
      !   WRITE(65,*) y_test_HT(k), v_test_HT(k,p), T_BL_HT(k,p) 
