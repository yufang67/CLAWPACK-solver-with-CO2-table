...
MODULE def_constants
  ...
  INTEGER, PARAMETER :: pr = selected_real_kind(12,307)  !< working precision consistent with AVBP (here double precision)
...
!  
  ...
  REAL(pr), PARAMETER :: v_D      = 5.51702987e-2_pr
!
  REAL(pr), PARAMETER :: T_E      = 278.2886799_pr        ! values computed by corner_E
  REAL(pr), PARAMETER :: v_E      = 8.68527009e-3_pr
!
  REAL(pr), PARAMETER :: e_F      = -111810.3809_pr   ! point F at saturation line before Umax
  REAL(pr), PARAMETER :: v_F      = 1.41627e-2_pr
  REAL(pr), PARAMETER :: P_F      = 1.905180e6_pr
!  
  REAL(pr), PARAMETER :: e_G      = -170e3_pr   ! point F at saturation line before Umax
  REAL(pr), PARAMETER :: v_G      = 2.126e-3_pr
!------------------------------------------------------------------------------------------------
!
