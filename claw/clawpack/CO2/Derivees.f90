...
! 
          IF (u_in .GT. u_end) THEN
             STOP '** Out of range. Too high specific internal energy, ####CO2DER####'
          ENDIF
!
...
!
          IF (v_in .LT. v_min) THEN
             STOP '** Out of range. Pressure higher than 50 MPa in HT, ####CO2DER####'
...
! In LL
           ...
           STOP '** Out of range. Too low specific internal energy value in LL,####CO2DER####'
           ENDIF  
!
...
!          
          IF (v_in .LT. v_min) THEN
             STOP '** Out of range. Pressure higher than 50 MPa in LL,####CO2DER####'
...
!
          IF (v_in .LT. v_min) THEN
             STOP '** Out of range. Pressure higher than 10 MPa in LH,####CO2DER####'
          ELSEIF (v_in .GT. v_sat) THEN
! Region Two-phase (HT)   
                flag_loca = 5
          ELSE
...
! In Solid phase, but we push it back to LP
         ...
         f_out = 2_pr
         Print*,'** out of range (SOLID) in  ####CO2DER#### case(6)'
        ENDIF
         gamma_pg = 1.313_pr
         e_pg     = 236.0294e3_pr
!
