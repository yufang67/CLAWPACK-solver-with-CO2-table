...
c
      if (maux .gt. 0)  then
c         print*, maxmx,maxmy,mbc,mx,my,xlower,ylower,dx,dy,
c     &               maux
         call setaux(maxmx,maxmy,mbc,mx,my,xlower,ylower,dx,dy,
     &               maux,aux)
c         print*,'clawez.f, setaux',aux_test(:,:,3)
         endif
c
...
c     # time increments between outputing solution:
      ...
      if (outstyle .eq. 1) then
         dtout = (tfinal - t0)/float(nout-n0+1)
         endif
...
c
      tend = t0
c      n0   = iframe*nstepout + 1
c      print*, 't0,dtouti,n0,nout',t0,dtout,n0,nout
      do 100 n=n0,nout
         tstart = tend
