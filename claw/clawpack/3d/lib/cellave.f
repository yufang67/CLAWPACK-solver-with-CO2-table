      ...
      external fs
      real(kind=2)  xlow,ylow,zlow,dx,dy,dz, w
...
c     # calling 'cellave' for each mesh cell should be okay.
      ...
      integer icube(0:7)
      real(kind=2) edges(0:7,0:7)

      integer ipt, ipt_cnt, iedge, ipt1, ipt2, i,j
      real(kind=2) xl,xh,yl,yh,zl,zh, tol, s
      real(kind=2) corners(0:7,3)

      logical all_in, all_out

c     # functions
      real(kind=2) zeroin, fs
...
c     # Computing area of polygon at x = 1
      real(kind=2) isect_list(10,3)
      real(kind=2) ypoly(10), zpoly(10), area
      integer iv_list(5)
...
c     # For triangulation of surface
      real(kind=2) p0(3), p1(3), p2(3)
      real(kind=2) dw, tri_normal(3), v1(3), v2(3)
      integer sgn, idir
...
c     # than criss-cross it.
      ...
      real(kind=2) function fs(s)
      implicit none

      real(kind=2) x,y,z, fdisc
      real(kind=2) xl,xh,yl,yh,zl,zh,s
