...
c ---------------------------------------------------------------
c
        subroutine filpatch4(level,nvar,valbig,aux,naux,
     1                      time,mitot,mjtot,
...
c  patch. otherwise, orig. caller of filpatch4 will take care of this
c
...
c boundary pieces filled.
        ...
        if (set) then
           ...
           write(outunit,*)" error in filpatch4 - level 1 not set"
           write(outunit,900) nrowst,ncolst
           write(*,*)" error in filpatch4 - level 1 not set"
...
c purely recursive formulation for interpolating.
        ...
        write(outunit,*)" 4 filpatches sufficient for properly",
     .                  " nested grids"
        write(*,*)" should not need recursive call here"
        write(*,*)" 4 filpatches sufficient for properly",
     .                  " nested grids"
        stop
...
c       coarsen
     ...
     &       sticksout(iplo,iphi,jplo,jphi)) then
             call prefil4(levc,nvar,alloc(loccrse),alloc(locauxc),naux,
     1                    time,nrowc,ncolc,1,1,
     2                    iplo,iphi,jplo,jphi)
        else
           call filpatch4(levc,nvar,alloc(loccrse),alloc(locauxc),naux,
     1                    time,nrowc,ncolc,1,1,
     2                    iplo,iphi,jplo,jphi)
        endif
